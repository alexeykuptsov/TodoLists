...
try
{
    ...
    dotnet ef migrations add CreateV1_0_0
    ThrowOnNativeFailure
    ...
    psql -h localhost -U todo_lists_app -p 5432 -d todo_lists -c "insert into projects (name, profile_id) values ('Inbox', (select id from profiles where name = 'dev'));"
    ThrowOnNativeFailure
    ...
    psql -h localhost -U todo_lists_app -p 5432 -d todo_lists -c "insert into projects (profile_id, name) values (1, 'Inbox');"
    ThrowOnNativeFailure
}
finally
{
